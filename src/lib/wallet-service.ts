...
// localStorage에서 게임 쿠폰 읽기 (CouponGame3D가 'my-coupons' 키로 저장)
function getLocalGameCoupons(): Coupon[] {
      ...
      try {
              ...
              return parsed.map((c: any) => ({
                        ...
                        brand: c.brand || c.storeName || c.store_name || 'CouponTalkTalk',
                        status: (c.status === 'active' || c.status === 'available') ? c.status : 'available' as Coupon['status'],
                        expiresAt: c.expiresAt || c.expires_at || new Date(Date.now() + 30 * 24 * 60 * 60 * 1000).toISOString(),
                        imageUrl: c.imageUrl || c.image_url,
                        discountRate: c.discountRate || c.discount_value || c.discount || c.points || 0,
              }));
      } catch (e) {
